    ...
    <div class="model-container">
        <el-card shadow="never">
            ...
            <el-row :gutter="15">
                ...
                <el-col :xl="4" :lg="6" :md="6" :sm="8">
                    <el-col class="input-name" :span="7">字典项值</el-col>
                    <el-col :span="17"><el-input placeholder="请输入关键字" v-model="seacherParam.itemValue" @keyup.enter.native="seachFun"></el-input></el-col>
                </el-col>
        ...
        <el-card>
            ...
            <div class="table-wrapper">
                <el-table empty-text="暂无数据" :stripe="true" :data="dataOption" v-loading="!dataOption">
                    ...
                    <el-table-column prop="dataDictCatCode" label="分类代码"></el-table-column>
                    
                    <el-table-column prop="itemValue" label="字典项值"></el-table-column>
                    ...
                    <el-table-column label="启用状态">
                     ...
                     <el-table-column label="启用/停用时间">
                        <template slot-scope="scope">
                            <span>{{scope.row.availableUpdateTime?fattTime(scope.row.availableUpdateTime):"暂无数据"}}</span>
                        </template>
                    </el-table-column>
        ...
        <el-row :gutter="15">
            ...
            <el-col :span="16" class="content-right">
              <el-select v-if="dialogType===1 || dialogType===2" size="small" v-model="dialogBindData.dataDictCatCode" placeholder="请选择">
                   <el-option
                   value="">
                   --请选择--</el-option>
                    <el-option
          ...
          <!-- <el-row :gutter="15">
            <el-col :span="8" class="content-left"><label>父级项</label></el-col>
            <el-col :span="16" class="content-right">
              <el-select v-if="dialogType===1 || dialogType===2" size="small" v-model="dialogBindData.parentItemId" placeholder="请选择">
                   <el-option value="">--请选择--</el-option>
                    <el-option
                    v-for="item in dataDictItems"
                    :key="item.dataDictItemId"
                    :label="item.itemValue"
                    :value="item.dataDictItemId">
                    </el-option>
                </el-select>

              <span v-else>{{dialogBindData.parentItemId}}</span>
            </el-col>
          </el-row> -->


          <el-row :gutter="15">
            <el-col :span="8" class="content-left"><label>字典项值</label></el-col>
            <el-col :span="16" class="content-right">
              <el-input v-if="dialogType===1 || dialogType===2" size="small" placeholder="请输入字典项值" v-model="dialogBindData.itemValue"></el-input>
              <span v-else>{{dialogBindData.itemValue}}</span>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
    seachFun() {
      this.$seach
        .submmit(this, system.getDataDictItemList)
        .then(res => {
    ...
    del(data) {
      system
        .delDataDictItem({
          dataDictItemId: data.dataDictItemId
        })
        .then(res => {
          if (res.data.code === "1") {
            this.getList(() => {
    ...
    submmitFun() {
      ...
      isSubmitActive = param.dataDictCatCode && param.itemValue;
      if (isSubmitActive) {
        this.submmitSuccess = true;
        system.addOrEditDataDictItem(param).then(res => {
          if (res.data.code == 1) {
            this.getList(() => {
              this.$message({
